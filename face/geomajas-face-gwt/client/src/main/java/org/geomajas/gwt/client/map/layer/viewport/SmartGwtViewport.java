package org.geomajas.gwt.client.map.layer.viewport;
...
/**
 * Copied from puregwt-client-impl org.geomajas.puregwt.client.map.ViewPortImpl
 * 
 * SmartGwt implementation of the GWT2 viewport. This is intended for internal use in the client WMS layer, as it does
 * not implement all ViewPort methods. It can however be extended to fully support everything.
 */
public class SmartGwtViewport implements ViewPort {
    ...
    public void initialize( ClientMapInfo mapInfo, MapEventBus eventBus ) {
        ...
        maxBounds = new Bbox( mapInfo.getMaxBounds().getX(), mapInfo.getMaxBounds().getY(),
                              mapInfo.getMaxBounds().getWidth(), mapInfo.getMaxBounds().getHeight() );
        ...
        if ( BboxService.equals( maxBounds, Bbox.ALL, 1e-10 ) ) {
            maxBounds = new Bbox( mapInfo.getInitialBounds().getX(), mapInfo.getInitialBounds().getY(),
        ...
        if ( mapInfo.getScaleConfiguration().getZoomLevels() != null
            ...
            zoomStrategy = new org.geomajas.gwt.client.map.layer.viewport.FixedStepZoomStrategy( mapInfo, maxBounds );
        } else {
            zoomStrategy = new org.geomajas.gwt.client.map.layer.viewport.FreeForAllZoomStrategy( mapInfo, maxBounds );
        }
    ...
    // }
    //
    // private MapView.ZoomOption convertZoomOption( org.geomajas.puregwt.client.map.ZoomStrategy.ZoomOption
    // zoomOption ) {
    // if ( zoomOption == null )
    ...
    // public void setMapSize( int width, int height ) {
    // if (this.mapWidth != width || this.mapHeight != height) {
    // Coordinate position = transform(new Coordinate(width / 2, height / 2), RenderSpace.SCREEN,
    // RenderSpace.WORLD);
    // this.mapWidth = width;
    // this.mapHeight = height;
    // if (zoomStrategy != null) {
    ...
    //
    // @Override
    // public org.geomajas.geometry.Bbox transform( org.geomajas.geometry.Bbox bbox, RenderSpace from, RenderSpace
    // to ) {
    // throw new UnsupportedOperationException();
    // }
    //
